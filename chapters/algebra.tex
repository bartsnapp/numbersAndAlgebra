...
\QM 
\begin{teachingnote}
Rather than grouping by tens and then by ten tens, Gertrude is grouping by fives and then by five fives.  So the numeral $234$ means $2$ groups of five groups of five, 3 groups of five, and 4 ones.  We call this approach ``base five,'' whereas our standard approach is called ``base ten.''  To indicate that this numeral is to be interpreted in base six, we might write $234_{five}$.  
\end{teachingnote}
...
\QM
\begin{teachingnote}
The Omicronians count in base six.  The six symbols above correspond to 
our digits $0$, $1$, $2$, $3$, $4$, and $5$, so we might write the numerals above 
as $134_{six}$ and $20_{six}$.  
\end{teachingnote}
...
\QM
\begin{teachingnote}
Your friend counts in base twelve, so in addition to the digits 0 through 9, they 
need two more symbols.  
\end{teachingnote}
...
\QM
\begin{teachingnote}
Addition is associative because $a+(b+c) = (a+b)+c$ always. 
Multiplication is associative because $a\cdot(b\cdot c) = (a\cdot b)\cdot c$ always.  

Subtraction is not associative because usually $a-(b-c) \ne (a-b)-c$.  
Division is not associative because usually $a\div (b\div c) \ne (a\div b)\div c$. 
\end{teachingnote}
...
\QM
\begin{teachingnote}
Addition is commutative because $a+b = b+a$ always. 
Multiplication is commutative because $a\cdot b= b \cdot a$ always.  

Subtraction is not commutative because usually $a-b \ne b-a$.  
Division is not commutative because usually $a\div b \ne b\div a$. 
...
\QM
\begin{teachingnote}
Multiplication distributes over addition because $a\cdot(b+c) = a\cdot b + a\cdot c$ and 
$(a+b)\cdot c = a\cdot c + b\cdot c$ always. 

But exponentiation does not distribute over addition because usually 
$a^{b+c} \ne a^b+ a^ c$ and 
$(a+b)^ c \ne a^ c + b^ c$. 
\end{teachingnote}
...
\QM
\begin{teachingnote}
Addition is closed on the counting numbers and on even numbers.  

Subtraction is not closed on the counting numbers.  For example $3-5$ is not a counting number.  
\end{teachingnote}

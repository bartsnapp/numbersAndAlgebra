...
Addition is probably the first operation we learn.  \fixnote{Distinguish negation from subtraction.}  
...
\item \label{P:NNP}Matt is a member of the Ohio State University
  ...
  is positive it means your step is a \textit{forward step of $y$
    ...
    backward step of $|y|$ inches.}\fixnote{We need additional models, e.g., checks and bills, red and black chips.}
\begin{enumerate}
...
\fixnote{Need to argue that the gcd is expressible this way.  Euclidean Algorithm did that.  Also, improve the corresponding activity.}
...
\fixnote{Include some problems about hall of shoes.} 
...
\fixnote{To emphasize that rational numbers are more precise than fraction, include $\pi/3$.}  
...
\fixnote{Use ``rational number'' here.  And write fractions vertically.}  
\begin{question}
...
\item How many rational numbers are there between $3/4$ and $4/7$?
  Find $3$ of them. Explain your reasoning. \fixnote{Maybe a sequence of rational numbers approaching something?}
...
\begin{question} Your calculator is not trustworthy for determining whether a number's decimal representation terminates or repeats.  Why?  
\end{question}
\QM
...
In this section, we instead write equations and solve---and these are skills that serve students well throughout school and college mathematics.  We build on understandings of fractions and prepare to understand functions and, importantly, to distinguish linear functions that are direct proportions from those that are not.  
...
\fixnote{The phrase ``use common sense'' is sometimes not helpful below.  Instead, record your guess and then reflect on the guess later.} 
...
\item Chris is working on his Fiat. His car's cooling system holds 6
  ...
  your reasoning. \fixnote{Make clear we start at 50/50, then add one quart}
...
\item Andy and Sandy run a race of a certain distance. Sandy finishes
  ...
  competition. Who wins this time?  \fixnote{Make clear that Andy finishes the first race and that Sandy has no head start.}
\begin{enumerate}
\item Before any computations are done, use common sense to guess the
  solution to this problem.
\item Try to get a feel for this problem by choosing numbers for the
...
\item Three drops of \textit{Monica's XXX Hot Sauce} were mixed with
  ...
  concentrations:\fixnote{Put in some help.}  
\begin{itemize}
...
\begin{activitynote}
Activities \ref{A:walk} through \ref{A:WorldSeries} are intended for this section.  
%%  Walk the Line
...
\item While waiting for their candidate to come in the auditorium to address the local constituency, the Mathematics Party members all shake hands with one another.  When Mark Pi walks in, he shakes hands with the big-wigs who are there.  A total of 6357 handshakes took place that day, and no one shook hands with the same person more than once.  How many people were there?  How many big-wigs were there?\fixnote{Make sure this is clear.} 
